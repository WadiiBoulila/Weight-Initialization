...
def evaluation_summary(dataset_name, model_name='', batch_size=16, avg_num=10, summary_save_path=log_path):
    # get path of all models that contains the entered init, model, and epochs 
    models_paths = [checkpoints_path + name for name in os.listdir(checkpoints_path) if dataset_name in name.split('-') and (model_name in name.split('-') or model_name == '')]
    print(models_paths)
    # get dataset object
    dataset = RemoteSensingDataset(dataset_name, batch_size, printing=False)
